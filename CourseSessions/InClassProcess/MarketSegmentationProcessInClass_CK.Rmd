...
author: "T. Evgeniou/Claudia/Pedro/Andy"
output:
  html_document:
...
*1) All correlations are below 0.7
*The highest correlations are
*.64 (Q14/Q13) This makes sense: People buy the latestet and greatest both for boat and accessories
*.63 (Q16/17) This makes sense: People tend to seek advice with the most knowledgable person in the group
*.58 (Q3/Q9) This makes sense: People who see boats as status symbols have to believe that the boat says something about what who they are 
* The correlations imply that the questions can likely be grouped without loosing to much information because people are likely to give the same answer to both questions
*
*
*
*
...
*The table and plot show how much of the variance is explained by each of the created components. We can learn that only 5 components haven an eigenvalue above 1 (this means that these 5 explain most of differences between the customer segments) By using these 5 components we can segment the customer base. By adding more components, we gain little value. The first component(eigenvalue 8.4) explains 30% of the variance. So, the first is by far the most important component. 
*Because the table is created with a function that orders the components from the one with the greatest explanatory power to the least, the plot will always have a similar shape.  
*
*Our goal is to reduce the complexity of the table. Thus, we would like to see that a small amount of components explains the biggest amount of the variance (ideally, we would like to have as little components as possible with a value above 1)
*
*
*
*
...
*Question 1)
*Component number 1: Passionate 
*Component number 2: Trendy, that wants last technology
*Component number 3: Outdoor enthusiast 
*Component number 4: Renaissance males
*Component number 5: Value conscious
*Component number 6: Do-it-yourself
*
*These components make business sense because the individual segements have similar needs and we can use the information to segment customers. We can e.g., identify the segments fairly easily, build a meaningful marketing campaign for each of the segments and target them via different channels. 
<hr>
*
*Question 2)
*We recommend to use 6 factors for the following reasons
*- from a statistical point of view, only 5 components have an eigenvalue above 1. This means, that the first 5 components explain almost everything. The 6th value still has an eigenvalue close to 1 (0.9), so it is still relevant, while the others are likely only noise. 6 components explain a total of 56% of the variance. 
*- from interpretation, we can build 6 meaningful segments (see question 1). The 7th component doesn't allow us to build another meaningful segment, so it is not justified
*- From a business perspective, you can market very efficiently to these segments. If you have less segments, you do not know your coustomers well enough to meet their needs; if you have more segments, you will get lost in too much complexity; e.g., you will not be able to put enough marketing budget to the individual segments.  
